...
def TEMP__execute(config_version, component_matrix_type):
    # load config
    config = utils.load_config(config_version)
    unity_env = config['unity_env']
    model_name = config['model_name']
    ...
    for i in range(n_components):
        for r in range(n_rotations):
            ...
            for c in range(len(['R', 'G', 'B'])):
                # normalize [0, 1]
                max_val = np.max(component_matrix_as_img_per_rot[:, :, c])
                min_val = np.min(component_matrix_as_img_per_rot[:, :, c])
                print(f'c: {c}, max_val: {max_val}, min_val: {min_val}')
                # component_matrix_as_img_per_rot[:, :, c] = \
                #     (component_matrix_as_img_per_rot[:, :, c] - min_val) / (max_val - min_val)
